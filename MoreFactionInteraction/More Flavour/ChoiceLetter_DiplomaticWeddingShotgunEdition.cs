...
{
    public class ChoiceLetter_DiplomaticWeddingShotgunEdition : ChoiceLetter
    {
        ...
        public override IEnumerable<DiaOption> Choices
        {
            get
            {
                ...
                else
                {
                    ...
                    DiaNode dialogueNodeAccept = new DiaNode(text: "MFI_AcceptedProposal".Translate(this.betrothedInternal, this.marriageSeekerInternal.Faction).CapitalizeFirst().AdjustedFor(this.marriageSeekerInternal));
                    ...
                    DiaOption reject = new DiaOption(text: "RansomDemand_Reject".Translate())
                    {
                        ...
                        {
                            //if (Rand.Chance(0.2f))
                            this.marriageSeekerInternal.Faction.TryAffectGoodwillWith(other: Faction.OfPlayer, goodwillChange: DiplomacyTuning.Goodwill_PeaceTalksBackfireRange.RandomInRange, canSendMessage: true, canSendHostilityLetter: true, reason: "LetterLabelRejectedProposal".Translate());
                            Find.LetterStack.RemoveLetter(this);
                        }
                    };
                    DiaNode dialogueNodeReject = new DiaNode(text: "MFI_DejectedProposal".Translate(this.marriageSeekerInternal.Name, this.marriageSeekerInternal.Faction).CapitalizeFirst().AdjustedFor(this.marriageSeekerInternal));
        ...
        private bool ColonyCanAffordDowry(Map map)
        {
            ...
            List<Thing> first = TradeUtility.AllLaunchableThingsForTrade(map) as List<Thing>;
            List<Thing> second = TradeUtility.AllSellableColonyPawns(map) as List<Thing>;
            first.AddRange(second);

            var uuuh = first.GroupBy(x => x.def, x => x.stackCount, (key, g) => new { ThingDef = key, stackCount = g });
